...
double HungarianAlgorithm::Solve(vector <vector<double> >& DistMatrix, vector<int>& Assignment)    //无匹配项则assignment为-1
{
	...
	//	
	//}
	//cout << "dadadadadada           " << max << endl;   //输出观察值


	////*********************************************
	////设置阈值，将距离权值较大的两个连接点强制断开
	////.............zeyi
	////*********************************************
	////
	//for (int i = 0; i < nRows; i++)
	//{
	//	int j = Assignment[i];
	//	if (j != -1 && DistMatrix[i][j] > REMOTE)         //注意: Assignment[i] 的值可能取到 -1 ，使得下标越界
	//	{
	//		Assignment[i] = -1;              //切断
	//	}
