...
Point previousPoint;
void line_mouse2(int event, int x, int y, int flags, void* ustc)
{
	...
	{
		Point pt(x, y);
		line(g_srcImage, previousPoint, pt, Scalar(0, 255, 0), 1, 8, 0);
		previousPoint = pt;
...
int main()
{
	...
	//g_srcImage = imread("D:\\sperm_0174.jpg");
	//g_srcImage.copyTo(g_dstImage);
	//imshow("WINDOW", g_srcImage);
	//setMouseCallback("WINDOW", line_mouse2, 0);
	//waitKey(0);

	//鼠标事件响应
	//org = imread("D:\\test.jpg");
	//org.copyTo(img);
	//org.copyTo(tmp);
	...
	//检测细胞重叠
	//Mat img = imread("D:\\sperm_0174.jpg");
	//if (img.empty())
	//{
	//	//检查是否读取图像
	//	cout << "error! 图片读取失败\n";
	...
	//Mat drawing = seg.segment("test", img, dst, temp_minellipse);
