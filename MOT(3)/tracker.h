...
struct obj
{
	...
	vector<int> pre_link;    //用来标记第i幅图片中每一个目标是否可作为轨迹段起点
};
struct tracklet      //轨迹段
{
...
public:
	...
	void form_trajectory();           //数据关联（对tracklets匈牙利算法匹配）
...
private:
	...
	vector<tracklet> tracklets;                 //未归类的轨迹段集合
...
public:
	...
	vector<vector<tracklet>> tracklets_sort;    //按照轨迹段的起始点进行了归类
	
	vector<vector<TimePosition>> motion_location;        //记录所有trajectory中每一时刻点的位置
};
